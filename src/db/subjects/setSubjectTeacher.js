...
export async function setSubjectTeacher(teacherId, subjectChanges) {
  try {
    ...
    const result = await Promise.all(
      ...
      subjectChanges.removedTaught.map(async (subjectId) => {
        const res =
          await sql`UPDATE subjects SET teacher_id = null WHERE id = ${subjectId};`;
      })
    );
