...
export async function createAssignment(
  ...
  try {
    ...
    let message;
    if (err.constraint === "assignment_dates_check") {
      message = "Assignment due date must be later than current datetime.";
    }
    if (err.constraint === "assignment_grade_check") {
      message = "Maximum grade must be a positive integer.";
    }

    return {
      success: false,
      message: message || "Error creating assignment.",
    };
  }
}
